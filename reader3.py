...
if __name__ == "__main__":
    ...
    import sys
    if len(sys.argv) < 2:
        print("Usage: python reader3.py <file.epub or directory>")
        sys.exit(1)
    ...
    if os.path.isdir(input_path):
        ...
        print(f"Batch processing EPUBs in directory: {input_path}")
        
        # List all files in the directory
        files = [f for f in os.listdir(input_path) if f.lower().endswith('.epub')]
        ...
        for f in files:
            epub_file = os.path.join(input_path, f)
            out_dir = os.path.splitext(epub_file)[0] + "_data"

            if os.path.exists(out_dir):
                print(f"Skipping {f} (Output directory exists)")
                continue
            ...
            try:
                ...
                print(f"Success: {book_obj.metadata.title}")
            except Exception as e:
                print(f"Error processing {f}: {e}")

    elif os.path.isfile(input_path):
        # Single file mode
        epub_file = input_path
        out_dir = os.path.splitext(epub_file)[0] + "_data"
